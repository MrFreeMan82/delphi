...
uses
  SysUtils, Classes, Windows, StrUtils, ConsoleRus in 'ConsoleRus.pas';
         ...
         begin
                ...
                begin
                     case fieldnum of
                            ...
                            4 : result.PYes       := PTReeElement(StrToInt(s));
                            5 : result.PNo        := PTReeElement(StrToInt(s));
                            6 : result.Parent     := PTreeElement(StrToInt(s));
                            7 : result.FirstChild := PTreeElement(StrToInt(s));
                            8 : begin
                                    s := s + ch;
                                    result.NextSibling := PTreeElement(StrToInt(s));
                                end;
                     end;
                     inc(fieldnum);
                     s := '';
...
procedure Read(var TreeElement : PTreeElement; Parent : PTreeElement; out Root : PTreeElement; out cnt : cardinal);
        ...
        begin
           if cardinal(Parent.NextSibling) = TreeElement.ID then
                                   Parent.NextSibling := TreeElement;

           if cardinal(Parent.PYes) = TreeElement.id then
                                  Parent.PYes := TreeElement;

           if cardinal(Parent.PNo) = TreeElement.ID then
                                  Parent.PNo  := TreeElement;

           if cardinal(Parent.FirstChild) = TreeElement.ID then
...
begin
    ...
    begin
        ...
        begin
           ...
           InitBranches();
           while cardinal(TreeElement.Parent) = Parent.ID do
           begin
                 ...
                 if cardinal(TreeElement.Parent) <> Parent.ID then exit;
           end;
...
try
   ...
   repeat
NextQuestion:
               WriteLn(Current.Question + '?');

               if Yes() then
               begin
                     WriteLN('Это - ' + Current.Answear + '. Угадала?');
                     if Yes then
                     begin
                           Writeln('Ура, Я выграла!!!');
                           Continue;
                     end
                     else begin
                           i := 1;
               ...
               begin
                    ...
                    Next.Parent := Current;
                    Next.ID     := cnt;
                    WriteLn('Сдаюсь, Кто это?');
                    ReadLn(Next.Answear);
                    Next.Answear := AnsiUpperCase(Next.Answear[1]) + RightStr(Next.Answear,length(Next.Answear) - 1);
                    WriteLn('Чем отличается ' + Current.Answear + ' от ' + Next.Answear + '?');
