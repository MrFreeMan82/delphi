...
const
  ActionStr: array[TReconcileAction] of string = ('Пропустить', 'Отменить обновление', 'Объединить',
    'Исправить', 'Отменить изменение', 'Обновить');
  UpdateKindStr: array[TUpdateKind] of string = ('Изменение', 'Добавление',
    'Удаление');
  SCaption = 'Ошибка - %s';
...
type
  ...
  public
    constructor CreateForm(DataSet: TDataSet; UpdateKind: TUpdateKind;
      Error: EReconcileError; customReason: String);
  end;

function HandleReconcileError(DataSet: TDataSet;  UpdateKind: TUpdateKind;
  ReconcileError: EReconcileError; customReason: String): TReconcileAction;
...
begin
  UpdateForm := TReconcileErrorForm.CreateForm(DataSet, UpdateKind, ReconcileError, customReason);
  with UpdateForm do
  try
...
constructor TReconcileErrorForm.CreateForm(DataSet: TDataSet;
  UpdateKind: TUpdateKind; Error: EReconcileError; customReason: String);
begin
  FDataSet := DataSet;
  FUpdateKind := UpdateKind;
  FError := Error;
  FCustomReason:=customReason;
...
begin
  ...
  OKBtn.Caption:='Отменить '+UpdateKindStr[FUpdateKind];
  ErrorMsg.Text := FError.Message;
  mmReason.Text:=FCustomReason;
