...
begin
  ...
  if Msg <> nil then
    raise ESqliteException.CreateFmt(string(s +'.'#13'Error [%d]: %s.'#13'"%s": %s'), [ret, SQLiteErrorStr(ret),SQL, Msg])
  else
    raise ESqliteException.CreateFmt(string(s), [SQL, 'No message']);
...
begin
  ...
  sql := 'select [sql] from sqlite_master where [type] = ''table'' and lower(name) = ''' +
    ansistring(lowercase(string(TableName))) + ''' ';
